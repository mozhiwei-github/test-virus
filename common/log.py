...
class Log(object):
    def __init__(self, casename="undefind", log_level=logging.INFO):
        self.caseid = os.environ.get("caseid")
        id = str(datetime.datetime.now().strftime('%Y%m%d%H%M%S')) + "." + str(round(time.time() * 1000))[-3:]
        if not self.caseid:
            self.caseid = "cid_" + id

        self.casename = os.environ.get("casename")
        ...
        # 日志文件处理器
        self.log_file = os.path.join(contants.logs_file, f"{self.caseid}.txt")
    ...
    def log_info(self, msg, title=None, attach=True, log_only=False, driver=None, screenshot=False, compress_rate=0.7,
                 shot_delay=0):
        """
        Info级别日志输出
        @param msg: 日志信息
        ...
        if title:
            ...
            if attach:
                if driver:
                    from common.utils import attach_driver_screenshot
                    time.sleep(shot_delay)
                    attach_driver_screenshot(driver, msg, compress_rate=compress_rate)
                elif screenshot:
                    from common.utils import attach_screenshot
                    time.sleep(shot_delay)
    ...
    def report(self, msg):
        ...
        response = requests.request("post", "http://autotest.cf.com/interface/pctestreportmark", json=data)
        res_json = json.loads(response.content)
        return res_json


# 从环境变量中读取日志等级
log_level = int(os.environ.get("UITEST_LOG_LEVEL", logging.INFO))
